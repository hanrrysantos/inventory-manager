...
import br.com.hanrry.inventory.exception.batch.InvalidStockException;
...
public class BatchService {
    ...
    public BatchResponseDTO createBatch(BatchRequestDTO request) {
        ...
        Product product = productRepository.findById(request.productId())
                .orElseThrow(() -> new ProductNotFound("Product not found with this id: " + request.productId())
                );
    ...
    public BatchResponseDTO addStockToBatch(Long id, Long quantity) {
        Batch batch = batchRepository.findById(id)
                .orElseThrow(() -> new BatchNotFound("Batch not found with this id: " + id));
    ...
    public void consumeStock(Long productId, Long totalQueOClienteQuer) {
        List<Batch> batches = batchRepository.findByProductIdAndQuantityGreaterThanOrderByExpiryDateAsc(productId, 0L);
    ...
    private void checkLowStockAndNotify(Long productId) {
        Product product = productRepository.findById(productId)
                .orElseThrow(() -> new ProductNotFound("Product not found with this id: " + productId)
                );
