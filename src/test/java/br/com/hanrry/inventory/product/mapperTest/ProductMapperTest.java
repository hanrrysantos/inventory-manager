...
class ProductMapperTest {
    ...
    @Test
    void shouldIncludeExpiredBatchesWhenCalculatingTotalQuantity() {
        Batch expiredBatch = new Batch();
        expiredBatch.setExpiryDate(java.time.LocalDate.now().minusDays(1));
        expiredBatch.setQuantity(4L);

        Batch validBatch = new Batch();
        validBatch.setExpiryDate(java.time.LocalDate.now().plusDays(1));
        validBatch.setQuantity(6L);

        Product product = new Product();
        product.setBatches(List.of(expiredBatch, validBatch));
